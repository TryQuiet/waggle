...
export class Tor {
  ...
  public async setSocksPort(port: number) {
    await this.torControl.setConf(`SocksPort="${port}"`)
  }

  public async setHttpTunnelPort(port: number) {
    await this.torControl.setConf(`HTTPTunnelPort="${port}"`)
  }

  public async addOnion({ port, privKey }) {
    const status = await this.torControl.addOnion(`${privKey} Flags=Detach Port=${port},127.0.0.1:${port}`
    )
  ...
  public async addNewService({ port }) {
    const status = await this.torControl.addOnion(
      `NEW:BEST Flags=Detach Port=${port},127.0.0.1:${port}`
    )
    console.log(status)
  ...
  public async deleteOnion({ serviceId }) {
    this.torControl.delOnion(``, function (err: any, status: any) {
      if (err) {
